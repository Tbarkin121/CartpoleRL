...
class ValueNet(torch.nn.Module):
    def __init__(self):
        ...
        layers = [nn.Linear(n_features, 64),
                  nn.LeakyReLU(),
                  nn.Linear(64, 64),  
                  nn.LeakyReLU(),
...
actor_optim = optim.Adam(Actor.parameters(), lr=1e-5)
#%%
...
for epoch in range(num_epochs):
    ...
    for i in range(num_q_holds):
        ...
        # for _ in range(minibatch_steps):
            ...
            action_pd_s1 = torch.distributions.Normal(probs_s1[:,:,0], 0.5*torch.ones_like(probs_s1[:,:,0].detach()))
            action_pd_s2 = torch.distributions.Normal(probs_s2[:,:,0], 0.5*torch.ones_like(probs_s2[:,:,0].detach()))
            ...
            if(policy_loss>10):
                print("large policy loss : {}, {}, {}".format(policy_loss, advantage, ratio))
                policy_loss_large_breakbullshit
        ...
        probs = Actor(s2)
        action_pd = torch.distributions.Normal(probs[:,:,0], 0.5*torch.ones_like(probs[:,:,0].detach()))
        # for _ in range(1):
    ...
    print(epoch)
    print('Policy Loss Avg: {}. Value Loss AVg: {}'.format(np.array(actor_loss_list).mean(), np.array(critic_loss_list).mean()))
...
for i in range(view_len):
    ...
    next_actions = probs[:,0,0].reshape(-1,1).detach()
