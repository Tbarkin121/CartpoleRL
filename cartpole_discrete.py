...
class CartPole():
    def __init__(self, num_envs=2, buf_horizon=10, gamma=0.9):
        ...
        # Pole Variables
        self.min_pole_mass = 0.05
        self.max_pole_mass = 0.05
        ...
        self.theta_threshold_radians = 30 * 2 * torch.pi / 360
    ...
    def step(self, actions):
        ...
        force = torch.where(actions==0, -self.force_scale, self.force_scale)
        ...
        if self.kinematics_integrator == 'euler':
            dxdt = torch.cat( (self.velocity.view((-1,1)), accel, self.omega.view((-1,1)), alpha), dim=1)
...
# #%%
# cp = CartPole(num_envs = 2, buf_horizon=20, gamma=0.9)
# cp.render_init()
# #%%
...
#     actions = torch.ones((cp.num_envs, 1))
#     cp.step(actions)
